...
export default class Cursor extends withBreakpointObserver(Base) {
  ...
  setSticky(target, isSticky, cursorStickySide = "center") {
    ...
    if (isSticky === undefined || isSticky === false) {
      ...
      const stickyTarget =
        target.querySelector("[data-cursor-target]") ?? target;
      const targetBox = stickyTarget.getBoundingClientRect();
      const cursorBox = getOffsetSizes(this.$refs.inner);
      const offsetX =
        cursorStickySide === "right"
          ? targetBox.width / 2 + cursorBox.width / 4
          : cursorStickySide === "left"
          ? -targetBox.width / 2
          : 0;
      const translateX =
        targetBox.x + targetBox.width / 2 - cursorBox.width / 2 + offsetX;
      const translateY =
        targetBox.y + targetBox.height / 2 - cursorBox.height / 2 + 6;
  ...
  onEnterLink({ target }) {
    ...
    const { cursorColor, cursorSticky, cursorStickySide } =
      this.currentTarget.element.dataset;
