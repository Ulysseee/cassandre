  ...
  hideShapes() {
    ...
    gsap.set(this.shapes, {
      strokeDasharray: (i, target) =>
        `${target.getTotalLength()} ${target.getTotalLength()}`,
      strokeDashoffset: (i, target) =>
        `${(this.$options.reverse ? -1 : 1) * target.getTotalLength()}`,
    });
  }

  drawStrokes(
    { duration = 1.3, delay = 0.6, ease = "power3.out" } = {
      duration: 1.3,
      delay: 0.6,
      ease: "power3.out",
    }
  ...
  progressDraw(progress) {
    gsap.set(this.shapes, {
      strokeDashoffset: (_, target) =>
        `${
          (this.$options.reverse ? -target.getTotalLength() : 0) +
          target.getTotalLength() * progress
        }`,
    });
  }
}
